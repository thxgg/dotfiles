...
```bash
bash "<this-skill-directory>/tools/validate.sh" diagram.mmd [output.svg]
```

Workflow:
1. Draft the diagram in a standalone `.mmd` file first.
2. Resolve `<this-skill-directory>` to the directory containing this `SKILL.md`, then run the validator.
