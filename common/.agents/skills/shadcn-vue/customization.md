...
Preset application can reinstall components and replace customized wrappers. Read repository instructions, inspect the existing design system, and obtain explicit approval before running `apply` in an established project. Prefer targeted CSS-variable changes when the project already has a customized component layer.
...
Or edit CSS variables directly in the `tailwindCssFile` reported by `shadcn-vue info`; do not assume it is named `globals.css`.
...
Existing wrappers and repository design-system rules take precedence over upstream defaults. Preserve typed local APIs, focus behavior, portal workarounds, and established visual tokens.
...
See [Updating Components in SKILL.md](./SKILL.md#updating-components) for the manual inspection and merge workflow.
