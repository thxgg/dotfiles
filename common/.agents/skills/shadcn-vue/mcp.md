...
Run from the project root with its package runner. For a pnpm project:

```bash
pnpm dlx shadcn-vue@latest mcp        # start the MCP server (stdio)
pnpm dlx shadcn-vue@latest mcp init   # write config for your editor
```
...
> **Tip:** MCP tools handle registry operations (search, view, install). For project configuration (aliases, framework, Tailwind version), use the project runner, e.g. `pnpm dlx shadcn-vue@latest info` — there is no MCP equivalent.
