...
async function extractPackedPackage(fixtureRoot) {
  ...
  // npm 12 keys JSON reports by package name; earlier npm versions use arrays.
  const entries = Array.isArray(report) ? report : Object.values(report);
  const entry = entries.find((item) => item?.name === "pi-mcp-adapter-local");
  assert.equal(typeof entry?.filename, "string", packed.stdout);
  const tarball = path.join(fixtureRoot, entry.filename);
  ...
  // npm workspaces hoist runtime dependencies above this private package.
  let dependencyRoot = process.cwd();
  while (!existsSync(path.join(dependencyRoot, "node_modules", "@modelcontextprotocol", "client"))) {
    const parent = path.dirname(dependencyRoot);
    assert.notEqual(parent, dependencyRoot, "MCP runtime dependencies must be installed");
    dependencyRoot = parent;
  }
  await symlink(path.join(dependencyRoot, "node_modules"), path.join(packageRoot, "node_modules"), "dir");
}
