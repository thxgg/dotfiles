...
export function loadMcpConfig(overridePath?: string, cwd = process.cwd()): McpConfig {
  ...
  const sources = getConfigSources(overridePath, cwd);
  const piProjectConfig = readValidatedConfig(
    getProjectPiConfigPath(cwd),
    `MCP config from ${getProjectPiConfigPath(cwd)}`,
  );
  const loadSharedProjectConfig = piProjectConfig?.settings?.loadSharedProjectConfig !== false;

  for (const source of sources) {
    if (source.id === "shared-project" && !loadSharedProjectConfig) continue;
    const loaded = source.id === "pi-project"
      ? piProjectConfig
