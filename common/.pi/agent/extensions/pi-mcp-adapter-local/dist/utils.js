...
export function interpolateRequiredEnvVars(value, label, environment = process.env) {
    const missing = getMissingEnvVars(value, environment);
    if (missing.length > 0) {
...
export function resolveServerUrl(definition, environment = process.env) {
    ...
    const resolved = interpolateRequiredEnvVars(definition.url, "MCP server URL", environment);
    try {
        new URL(resolved);
    }
    catch (error) {
