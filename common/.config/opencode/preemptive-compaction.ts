...
const CLAUDE_OPUS_4_PATTERN = /claude-opus-4/i;

const CONTEXT_LIMIT_1M = 1_000_000;
const CONTEXT_LIMIT_200K = 200_000;
...
function getContextLimit(modelID: string): number {
    if (CLAUDE_OPUS_4_PATTERN.test(modelID)) {
        return CONTEXT_LIMIT_1M;
    }
    return CONTEXT_LIMIT_200K;
}
    ...
    return tool({
        ...
        async execute() {
            ...
            const lines = [
                ...
                `  threshold: ${threshold}`,
                `  context limits: opus-4 = ${CONTEXT_LIMIT_1M.toLocaleString()}, others = ${CONTEXT_LIMIT_200K.toLocaleString()} tokens`,
