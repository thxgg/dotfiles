#!/usr/bin/env bash
#
# package-skill.sh - Package an OpenCode skill into a distributable zip file
#
# Usage: package-skill.sh <skill-dir> [output-dir] [--dry-run]
...
Examples:
  package-skill.sh ./my-skill
  package-skill.sh ./my-skill ./dist
  package-skill.sh ./my-skill --dry-run
EOF
    exit 1
}
...
if [[ -x "$VALIDATE_SCRIPT" ]]; then
    ...
    echo -e "${YELLOW}Warning${NC}: validate-skill.sh not found, skipping validation"
fi
echo ""
...
# Count files and show what will be packaged
...
(cd "$PARENT_DIR" && zip -r "$ZIP_FILE" "$SKILL_NAME" "${EXCLUDE_ARGS[@]}" -x "*.git*" > /dev/null)

echo -e "${GREEN}Done.${NC} Created $ZIP_FILE"
echo ""
