#!/usr/bin/env bash
#
# init-skill.sh - Scaffold a new OpenCode skill directory
#
# Usage: init-skill.sh <skill-name> <output-dir> [--type TYPE] [--minimal]
#
...
usage() {
    cat << 'EOF'
Usage: init-skill.sh <skill-name> <output-dir> [options]
...
Examples:
  init-skill.sh my-skill ~/.config/opencode/skills
  init-skill.sh my-skill .opencode/skills --type reference-heavy
  init-skill.sh my-skill ./skills --minimal
EOF
    exit 1
}
...
| File | Purpose |
|------|---------|
| [api.md](./references/api.md) | TODO: API documentation |
| [patterns.md](./references/patterns.md) | TODO: Best practices |
| [gotchas.md](./references/gotchas.md) | TODO: Common issues |
...
```bash
bash $SKILL_DIR/scripts/example.sh <args>
```
EOF
            ;;
    esac
...
echo ""
echo -e "${GREEN}Done.${NC} Skill created at $SKILL_DIR"
echo ""
echo "Next steps:"
...
echo "  4. Validate: bash <skill-creator-dir>/scripts/validate-skill.sh $SKILL_DIR"
