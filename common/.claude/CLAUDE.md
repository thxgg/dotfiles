...
When the current model is Fable 5 and the agent wants to spawn subagents, always use **Pi with GPT-6 Sol** (`openai-codex/gpt-6-sol`) via subprocess — do not use the built-in Agent tool models.
...
```
pi -p --model openai-codex/gpt-6-sol --thinking high --no-session <<'EOF'
<self-contained task brief here>
EOF
```

Use the current repo as cwd. The model/effort must be Codex GPT-6 Sol with high thinking:
`--model openai-codex/gpt-6-sol --thinking high`

For stateful/multiturn integration, start RPC mode:

```
pi --mode rpc --model openai-codex/gpt-6-sol --thinking high --name "claude-subagent"
```
...
## Mixed-model workflows when running as GPT-6 Sol

When the current model is GPT-6 Sol inside Claude Code, use native dynamic workflows for tasks where independent analysis materially improves confidence, such as architecture decisions, difficult debugging, plans, and reviews.

For those workflows:

- Run independent analysis branches with both `gpt-6-sol` and `claude-fable-5`, normally at high effort.
